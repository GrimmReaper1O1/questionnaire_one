...
if (isset($_GET['reset'])) {
	...
	<form action="division.php?numbers=yes" method="POST">
...
if (isset($_GET['numbers'])) {
	...
	?>
	<div class="width50">
	<form action="division.php?calculate=yes" method="POST">
	<?php
	...
	for($i = 1; $i < $questions; $i++) {
		...
		$_SESSION['top' . $i] = $top;
		$_SESSION['bottom' . $i] = $bottom;
	
		echo strval($_SESSION['top' . $i]) . " /" . strval($_SESSION['bottom' . $i])  . "<br>";
		$s1 = '';
		$s2 = '';
	...
	<div class="centeredText">
	<form action="division.php?reset=yes" method="POST">
